...
impl TypeEnvironment {
    ...
    pub fn create_type(&mut self, name: &str) -> Option<TypeReference> {
    ...
    pub fn find_type_or_err(&self, reference: TypeReference) -> Option<&TypeDefinition> {
        let id = reference.get_id();
        if id >= self.types.len() {
            return None;
        }
    ...
    pub fn find_type_mut_or_err(&mut self, reference: TypeReference) -> Option<&mut TypeDefinition> {
        let id = reference.get_id();
        if id >= self.types.len() {
            return None;
        }
