...
use ::passes::ast::code_generation::spirv_symbol_table::SpirvSymbolTable;
use ::passes::ast::code_generation::spirv_type_cache::SpirvTypeCache;
...
impl GenerateSpirvPass {
    pub fn new(symbol_table: SymbolTableReference, result: PassResultReference) -> GenerateSpirvPass {
    ...
    // TODO add capability to extract this from the type-erased form Pass<Ast>
    // probably create a struct to report errors to and add results to that
    // gets passed as Rc<T> into every pass
    ...
    fn get_spirv_type(&mut self, type_ref: Option<TypeReference>) -> spirv::Word {
        ...
        let spirv_type = match self.symbol_table.borrow().find_type(t) {
...
mod tests {
    ...
    pub fn it_works() {
        ...
        let symbol_table = SymbolTableReference::new(compilation.get_symbol_table());
        let result = PassResultReference::new(PassResult::new());
        let mut pass = GenerateSpirvPass::new(symbol_table, result);
